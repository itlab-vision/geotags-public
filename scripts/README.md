...
Для запуска необходимо передать путь к базе достопримечательностей города (ключ `-i`),
путь к файлу с атрибутами (ключ `-a`) и путь с названием файла, куда будет сохранена 
отфильтрованная база (ключ `-o`).
...
### Скрипт переноса альтернативных названий

Скрипт выполняет поиск соответствий между городами двух баз. Первая база не содержит альтернативные названия
городов, вторая - содержит. Поиск соответствий осуществляется по координатам и имени с дальнейшим добавлением
альтернативных названий городов в первую базу.

```bash
python3 ./scripts/join_cities_altnames.py  -c <path/to/cities_db> \
                                           -a <path/to/cities_with_altnames_db> \
                                           -o <path/to/output_cities_altnames_db> \
                                           -d <max_distance> \
                                           -f <fuzzy_threshold>
```

Для запуска необходимо передать путь до основной базы городов (ключ `-c`), 
путь до базы городов с их альтернативными названиями (ключ `-a`), и 
путь с именем файла, куда будет сохранён результат (ключ `-o`). 
Также можно указать максимальное допустимое расстояние между городами (ключ `-d`) и 
минимально допустимое значение коэффициента сходства названия городов (ключ `-f`).
